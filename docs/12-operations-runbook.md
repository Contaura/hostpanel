...
AUTH_TOKEN="***"
curl -sf -H "Authorization: Bearer TOKEN" http://localhost:3001/api/health/readiness
...
```bash
AUTH_TOKEN="***"
curl -sf -H "Authorization: Bearer TOKEN" http://localhost:3001/api/health/live
```
...
4. Verify readiness returns only the expected advisory until TOTP is re-enabled:
   ```bash
   AUTH_TOKEN="***"
   curl -sf -H "Authorization: Bearer TOKEN" http://localhost:3001/api/health/readiness | jq '.checks.security'
   ```
