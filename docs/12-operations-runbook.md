...
```bash
ADMIN_JWT="paste-admin-jwt-here"
curl -sf -H "Authorization: Bearer $ADMIN_JWT" http://localhost:3001/api/health/live
```
...
4. Verify readiness returns only the expected advisory until TOTP is re-enabled:
   ```bash
   ADMIN_JWT="paste-admin-jwt-here"
   curl -sf -H "Authorization: Bearer $ADMIN_JWT" http://localhost:3001/api/health/readiness | jq '.checks.security'
   ```
