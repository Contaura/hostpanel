...
cPanel/WHM is still broader and older, so the remaining gap is mostly production depth: richer analytics/report exports, extending the centralized background-job/progress subsystem into the remaining app-installer/edge operations, and ongoing hardening/regression coverage.
...
| Databases | MySQL Databases, Database Users, phpMyAdmin | Strong foundation | Database management, phpMyAdmin install/detection, account-scoped links, and short-lived Signon handoff endpoint are implemented. Live Signon validation is covered by the managed validation endpoint and production checklist after distro package updates. |
...
1. Extend centralized background-job/progress/log tracking into the remaining app-installer and smaller edge operations; scanner jobs, backup create/restore, transfer execution, DNS sync, WebDAV provision/reload, and plugin install/rollback are already wired.
2. Deepen metrics/mail reports with charts, exports, retention, and more filters.
3. Continue adding account-scope/team-subaccount regression tests as portal modules grow.
4. Keep hardening service-command routes to the safe argv/Node-primitive pattern and avoid shell pipelines.
