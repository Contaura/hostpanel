...
describe('production health and readiness checks', () => {
  ...
  it('adds a manual launch blocker when PayPal live payments are configured without webhook verification evidence in production', async () => {
    const prevNodeEnv = process.env.NODE_ENV;
    const prevSshdConfig = process.env.SSHD_CONFIG_FILE;
    const prevRequiredServices = process.env.READINESS_REQUIRED_SERVICES;
    const prevDrillReportDir = process.env.DRILL_REPORT_DIR;
    ...
    await fs.writeFile(sshdConfig, 'PasswordAuthentication no\n');
    process.env.SSHD_CONFIG_FILE = sshdConfig;
    process.env.BACKUP_DIR = path.join(tmp, 'backups-paypal');
    process.env.DRILL_REPORT_DIR = path.join(process.env.BACKUP_DIR, 'drills');
    await fs.mkdir(process.env.DRILL_REPORT_DIR, { recursive: true });
    const freshReport = path.join(process.env.DRILL_REPORT_DIR, 'fresh-drill.json');
    await fs.writeFile(freshReport, JSON.stringify({ success: true, drill: true, backup: 'files_all_2026-06-01T00-00-00.tar.gz', verifiedAt: new Date().toISOString(), archive: { size: 6, sha256: 'f0355b03869c88f43e2f0d1049013088a8fdd522ff572c871be7bdac22545477' }, restorePlan: { dryRun: true, type: 'files', actions: ['Would restore ./index.html'] } }));
    ...
    db.prepare('INSERT INTO backup_schedules (type, target, schedule, enabled) VALUES (?, ?, ?, ?)')
      .run('database', 'hostpanel', '0 2 * * *', 1);
    db.prepare("UPDATE settings SET value = ? WHERE key = 'paypal_client_id'").run('live-client-id');
    db.prepare("UPDATE settings SET value = ? WHERE key = 'paypal_secret'").run('live-secret');
    db.prepare("UPDATE settings SET value = ? WHERE key = 'paypal_mode'").run('live');
    db.prepare("INSERT INTO settings (key, value) VALUES (?, '') ON CONFLICT(key) DO UPDATE SET value=excluded.value").run('paypal_webhook_verified');
    ...
    try {
      ...
      expect(res.status).toBe(200);
      expect(body.ok).toBe(true);
      expect(body.checks.payments).toMatchObject({ ok: true, paypalConfigured: true, paypalLiveMode: true, paypalWebhookVerified: false });
      expect(body.launchBlockers).toEqual([
        expect.objectContaining({ code: 'payment_webhook_secrets_unverified', severity: 'manual', owner: 'Marcos', requiredEvidence: expect.stringMatching(/PayPal webhook verification/i), message: expect.stringMatching(/PayPal live payments are configured without webhook verification evidence/i) }),
      ]);
    } finally {
      await server.close();
      process.env.NODE_ENV = prevNodeEnv;
