...
import { Plus, Trash2, Play, Tag } from 'lucide-react';
import { useToast } from '../components/Toast';

const api = (p: string, o?: RequestInit) => fetch(`/api/billing${p}`, { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('token')}` }, ...o });
            ...
            <tbody>
              ...
              {credits.map((c: any) => (
                <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="table-cell"><button className="btn-icon text-red-500" onClick={() => { api(`/credit-notes/${c.id}`, { method: 'DELETE' }); setCredits(cc => cc.filter(x => x.id !== c.id)); }}><Trash2 size={13} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
